...
class Match:
    ...
    def start_innings(self):
        ...
        self.inning1 = Inning(self.number_of_overs, self.number_of_players, self.team1.players, self.team2.players);
        while (not self.inning1.is_over):
            self.inning1.update_inning(input());
        print("***** End first innings *****");

        print("***** Start second innings *****");
        self.inning2 = Inning(self.number_of_overs, self.number_of_players, self.team2.players, self.team1.players,
                              self.inning1.runs_scored);
